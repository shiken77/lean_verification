...
## Deploy a public demo on Render (optional password)

The owner has chosen to publish this **experimental research demo without a login**, including the DeepSeek and Lean workflow. The Docker image and Blueprint set `APP_REQUIRE_AUTH=0`. Anyone who can reach the site can start model runs against the owner's API credit and access results if they have the result URL. There are no separate user accounts or private result storage. Do not enter secrets or sensitive data.

**Public access is a usability choice, not a security guarantee.** Limited model credit caps neither all infrastructure risks nor hostile code execution. This prototype still lacks a fully isolated Lean worker. Use a dedicated API key, monitor usage, and stop the service when the trial is over. Retained comparison reports are also visible without login. Password protection remains optional and does not itself sandbox Lean.
...
3. For password-free access, set `APP_REQUIRE_AUTH=0` and **remove `APP_ACCESS_PASSWORD`** if you added it earlier. A nonempty `APP_ACCESS_PASSWORD` still enables login even when `APP_REQUIRE_AUTH=0`. The Docker image already supplies `APP_REQUIRE_AUTH=0`, but an old value in Render's settings overrides the image. Use HTTPS.
...
6. Create the web service. Once it is **Live**, open the exact HTTPS URL shown by Render in a private/incognito browser window. It should open without a login. Run the built-in maximum demo and confirm the real Lean loop reports FAIL → PASS. Select DeepSeek to use model-generated answers after setting the API key. Share the website URL, never the API key.
...
To restore password protection later, set `APP_REQUIRE_AUTH=1` and `APP_ACCESS_PASSWORD` to a unique random password of at least 16 characters, then redeploy. The website username is `guest`. To update an existing deployment to the new public version, remove the old password, set `APP_REQUIRE_AUTH=0`, and rebuild/deploy the latest `main` commit; restarting an old image is insufficient. Blueprint updates do not automatically remove secrets already stored in Render.

### Hosted-demo defaults and limits

- The container runs as a non-root user and binds to `0.0.0.0:$PORT` (default `10000`). Local `python3 app.py` remains on `127.0.0.1:8765`. Non-local binding without a password requires the explicit `APP_REQUIRE_AUTH=0` setting supplied by the Docker image. If authentication is required or a password is configured, the password must be at least 16 characters.
...
- All app pages and status endpoints are accessible without login by default in the Render image; an optional configured password protects them. Forms still require a per-process anti-forgery token. Opening a link cannot start a paid benchmark; starting work requires a form submission. These checks do not prevent an anonymous visitor from deliberately starting repeated runs.
...
- Lean subprocesses receive an allowlisted environment, without the API key or web password. **This is not a security boundary**: Lean and Python still share an OS user and filesystem. Source regex checks, a container, and a password do not protect against all hostile Lean programs. A hardened public service would need a separately isolated verifier worker with no secrets, network restrictions and resource limits; that isolation has not been implemented here.
...
GitHub Actions tests the code and Docker image, including both public and optional password-protected access, without real API credentials. Render hosts the running app and can [deploy after CI checks pass](https://render.com/docs/deploys). GitHub Pages is not used for this backend.
